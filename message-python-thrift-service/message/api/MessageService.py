...
class Iface(object):
    ...
    def sendEmailMessage(self, eamil, message):
        """
        Parameters:
         - eamil
         - message
...
class Client(Iface):
    ...
    def sendEmailMessage(self, eamil, message):
        ...
        Parameters:
         ...
         - message
        """
        self.send_sendEmailMessage(eamil, message)
        return self.recv_sendEmailMessage()

    def send_sendEmailMessage(self, eamil, message):
...
class Processor(Iface, TProcessor):
    ...
    def process_sendEmailMessage(self, seqid, iprot, oprot):
        ...
        result = sendEmailMessage_result()
        try:
            result.success = self._handler.sendEmailMessage(args.eamil, args.message)
            msg_type = TMessageType.REPLY
        except TTransport.TTransportException:
            raise
        except TApplicationException as ex:
...
class sendEmailMessage_args(object):
    ...
    def __init__(self, eamil=None, message=None,):
        self.eamil = eamil
        self.message = message
    ...
    def read(self, iprot):
        ...
        while True:
            ...
            if fid == 1:
                if ftype == TType.STRING:
                    self.eamil = iprot.readString().decode('utf-8') if sys.version_info[0] == 2 else iprot.readString()
                else:
                    iprot.skip(ftype)
            elif fid == 2:
                if ftype == TType.STRING:
    ...
    def write(self, oprot):
        ...
        if self.eamil is not None:
            oprot.writeFieldBegin('eamil', TType.STRING, 1)
            oprot.writeString(self.eamil.encode('utf-8') if sys.version_info[0] == 2 else self.eamil)
            oprot.writeFieldEnd()
        if self.message is not None:
            oprot.writeFieldBegin('message', TType.STRING, 2)
            oprot.writeString(self.message.encode('utf-8') if sys.version_info[0] == 2 else self.message)
...
sendEmailMessage_args.thrift_spec = (
    None,  # 0
    (1, TType.STRING, 'eamil', 'UTF8', None, ),  # 1
    (2, TType.STRING, 'message', 'UTF8', None, ),  # 2
)
